...
const base = (index: number) => ({
  ...
  titleSpanish: _mock.postTitle(index),
  description: _mock.description(index),
  descriptionSpanish: _mock.description(index),
  favorited: _mock.boolean(index),
  createdAt: _mock.time(index),
  duration: '8 minutos de lectura',
  tags: _tags.slice(index + 1, index + 2),
  author: {
    name: _mock.fullName(index),
    role: _mock.role(index),
    avatarUrl: _mock.image.avatar(index),
    quotes: 'Miembro desde Mar 15, 2021',
    about:
...
export const _marketingPosts = [...Array(12)].map((_, index) => ({
  ...
  contentSpanish: content('marketing'),
  category: 'marketing',
