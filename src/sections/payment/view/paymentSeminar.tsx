...
function PayPalCheckoutSeminar({ prod }: Props) {
  ...
  async function updateOrCreateOrder() {
    ...
    if (existingOrder) {
      const updatedSlugs = [...existingOrder.seminario_slugs, prod.description];
      const updatedOrder = {
        ...existingOrder,
        seminario_slugs: updatedSlugs,
      };
      await client.createOrReplace(updatedOrder);
      console.log('Updated order with new seminario slug');
    } else {
      // Create a new order document if it doesn't exist
      const newOrder = {
        _type: 'scheduledseminars',
        email,
        seminario_slugs: [prod.description],
      };
...
export default PayPalCheckoutSeminar;
